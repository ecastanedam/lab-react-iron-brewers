...
function BeerDetailsPage() {
  ...
  const {beerId} = useParams(); // 1. Get the beer ID from the URL, using the useParams hook.

  useEffect( () => { //
    axios
    .get (`https://ih-beers-api2.herokuapp.com/beers/${beerId}`) // 3. Use axios to make a HTTP request
    .then ((response) => {
      setBeer(response.data); // 4. Use the response data from the Beers API to update the state variable.
    })
    .catch((error) => {
      console.error(error);
    });
